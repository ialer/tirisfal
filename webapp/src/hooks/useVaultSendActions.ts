...
import {
  attachTirisfalEncryptedAttachmentPayload,
  buildAccountEncryptedBitwardenJsonString,
  buildBitwardenZipBytes,
  buildExportFileName,
  buildTirisfalAttachmentRecords,
  buildTirisfalPlainJsonDocument,
      ...
      async exportVault(request: ExportRequest) {
        ...
        if (format === 'bitwarden_json') {
          ...
          if (request.encryptedJsonMode === 'password') {
            ...
            const [encryptedJson, attachments] = await Promise.all([getEncryptedJson(), getTirisfalAttachmentRecords()]);
            const withAttachments = await attachTirisfalEncryptedAttachmentPayload(
              encryptedJson,
              attachments,
              session.symEncKey!,
              session.symMacKey!
