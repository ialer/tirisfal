...
export default function PublicSendPage(props: PublicSendPageProps) {
  ...
  async function downloadFile(): Promise<void> {
    ...
    try {
      if (IS_DEMO_MODE) {
        const bytes = new TextEncoder().encode('Tirisfal demo file Send.\nThis download is generated locally in demo mode.\n');
        downloadBytesAsFile(bytes, sendData.decFileName || sendData.file?.fileName || 'tirisfal-demo-send.txt', 'application/octet-stream');
        return;
      }
  ...
  return (
    <div className="auth-page public-send-page">
      <StandalonePageFrame
        title={sendData ? (sendData.decName || t('txt_no_name')) : t('txt_tirisfal_send')}
        eyebrow={sendData ? t('txt_tirisfal_send') : undefined}
      >
