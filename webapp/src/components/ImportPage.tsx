...
interface PendingPasswordImportContext {
  parsed: BitwardenPasswordProtectedInput;
  source: 'bitwarden_json' | 'tirisfal_json' | 'bitwarden_zip';
  attachments: ImportAttachmentFile[];
}
...
function parseTirisfalAttachmentArray(raw: unknown): ImportAttachmentFile[] {
...
export default function ImportPage({ onImport, onImportEncryptedRaw, accountKeys, onNotify, folders, onExport }: ImportPageProps) {
  ...
  async function extractTirisfalAttachments(parsed: unknown): Promise<ImportAttachmentFile[]> {
    if (!isRecord(parsed)) return [];
    const direct = parseTirisfalAttachmentArray(parsed.tirisfalAttachments);
    if (direct.length) return direct;

    const encryptedPayload = String(parsed.tirisfalAttachmentsEnc || '').trim();
    ...
    return parseTirisfalAttachmentArray(unpacked.tirisfalAttachments);
  }

  async function runTirisfalJsonImport(parsed: unknown, extraAttachments: ImportAttachmentFile[] = []): Promise<ImportResultSummary> {
    const bundled = await extractTirisfalAttachments(parsed);
  ...
  async function processPasswordProtectedImport(ctx: PendingPasswordImportContext): Promise<ImportResultSummary> {
    ...
    if (ctx.source === 'tirisfal_json') {
      return runTirisfalJsonImport(parsed, ctx.attachments);
    }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (source === 'bitwarden_json' || source === 'tirisfal_json') {
        let parsed: unknown;
        try {
        ...
        const summary =
          source === 'tirisfal_json'
            ? await runTirisfalJsonImport(parsed)
  ...
  const exportNeedsMode =
    ...
    exportFormat === 'tirisfal_encrypted_json';
