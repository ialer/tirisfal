...
import { MachineAccount, SmProject, Secret, SecretAccessLog, CreateMachineAccountRequest, CreateProjectRequest, CreateSecretRequest, MachineAccountTokenResponse } from '../types/sm';
...
export class SecretsManagerService {
  ...
  async createMachineAccount(userId: string, request: CreateMachineAccountRequest): Promise<MachineAccount> {
  ...
  async getMachineAccount(id: string): Promise<MachineAccount | null> {
    const result = await this.db.prepare(
      'SELECT * FROM machine_accounts WHERE id = ?'
    ).bind(id).first<MachineAccount>();
    return result || null;
  }

  async getMachineAccountsByUserId(userId: string): Promise<MachineAccount[]> {
    const result = await this.db.prepare(
      'SELECT * FROM machine_accounts WHERE user_id = ? ORDER BY created_at DESC'
    ).bind(userId).all<MachineAccount>();
    return result.results || [];
  }

  async updateMachineAccount(id: string, updates: Partial<MachineAccount>): Promise<MachineAccount | null> {
    const now = new Date().toISOString();
    ...
    await this.db.prepare(
      `UPDATE machine_accounts SET ${setClauses.join(', ')} WHERE id = ?`
    ).bind(...values).run();
  ...
  async deleteMachineAccount(id: string): Promise<boolean> {
    const result = await this.db.prepare(
      'DELETE FROM machine_accounts WHERE id = ?'
    ).bind(id).run();
  ...
  async generateAccessToken(machineAccountId: string): Promise<MachineAccountTokenResponse> {
    ...
    ).bind(hashHex, expiresAt, new Date().toISOString(), machineAccountId).run();
  ...
  async verifyAccessToken(token: string): Promise<MachineAccount | null> {
    ...
    ).bind(hashHex, 'active', new Date().toISOString()).first<MachineAccount>();
  ...
  async getProject(id: string): Promise<SmProject | null> {
    const result = await this.db.prepare(
      'SELECT * FROM sm_projects WHERE id = ?'
    ).bind(id).first<SmProject>();
    return result || null;
  }

  async getProjectsByUserId(userId: string): Promise<SmProject[]> {
    const result = await this.db.prepare(
      'SELECT * FROM sm_projects WHERE user_id = ? ORDER BY created_at DESC'
    ).bind(userId).all<SmProject>();
    return result.results || [];
  }

  async deleteProject(id: string): Promise<boolean> {
    const result = await this.db.prepare(
      'DELETE FROM sm_projects WHERE id = ?'
    ).bind(id).run();
    return result.meta.changes > 0;
  }
  ...
  async getSecretsByProject(projectId: string, environment?: string): Promise<Secret[]> {
    ...
    const result = await this.db.prepare(query).bind(...params).all<Secret>();
    return result.results || [];
  }

  async getSecretByNameAndProject(name: string, projectId: string, environment: string): Promise<Secret | null> {
    const result = await this.db.prepare(
      'SELECT * FROM secrets WHERE name = ? AND project_id = ? AND environment = ?'
    ).bind(name, projectId, environment).first<Secret>();
    return result || null;
  }
  ...
  async grantProjectAccess(machineAccountId: string, projectId: string, permission: string = 'read'): Promise<void> {
    const now = new Date().toISOString();
    await this.db.prepare(
      'INSERT OR REPLACE INTO machine_account_projects (machine_account_id, project_id, permission, created_at) VALUES (?, ?, ?, ?)'
    ).bind(machineAccountId, projectId, permission, now).run();
  }
  ...
  async revokeProjectAccess(machineAccountId: string, projectId: string): Promise<boolean> {
    ...
    ).bind(machineAccountId, projectId).run();
    return result.meta.changes > 0;
  }

  async getProjectAccess(machineAccountId: string): Promise<{project_id: string, permission: string}[]> {
    const result = await this.db.prepare(
      'SELECT project_id, permission FROM machine_account_projects WHERE machine_account_id = ?'
    ).bind(machineAccountId).all<{project_id: string, permission: string}>();
    return result.results || [];
  }
  ...
  async hasProjectAccess(machineAccountId: string, projectId: string): Promise<boolean> {
    ...
    ).bind(machineAccountId, projectId).first<{count: number}>();
  ...
  async logSecretAccess(
    ...
    ).bind(id, machineAccountId, userId, secretId, action, ipAddress, userAgent, now).run();
  }
  ...
  async getSecretAccessLogs(secretId: string, limit: number = 100): Promise<SecretAccessLog[]> {
    ...
    ).bind(secretId, limit).all<SecretAccessLog>();
    return result.results || [];
  }
}
